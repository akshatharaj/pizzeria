...
namespace Pages
{
    public partial class Pages_Shop : System.Web.UI.Page
    {
        ...
        {
            //Get all pizza Objects from database
            ArrayList pizzaList = ConnectionClass.GetPizzaByType("%");

            foreach (Pizza pizza in pizzaList)
            {
                //Create Controls
                Panel pizzaPanel = new Panel();
                Image image = new Image { ImageUrl = pizza.Image, CssClass = "ProductsImage" };
                Literal literal = new Literal { Text = "<br />" };
                Literal literal2 = new Literal { Text = "<br />" };
                Label lblName = new Label { Text = pizza.Name, CssClass = "ProductsName" };
                Label lblPrice = new Label
                                     {
                                         Text = String.Format("{0:0.00}", pizza.Price + "<br />"),
                ...
                pizzaPanel.Controls.Add(image);
                pizzaPanel.Controls.Add(literal);
                pizzaPanel.Controls.Add(lblName);
                pizzaPanel.Controls.Add(literal2);
                pizzaPanel.Controls.Add(lblPrice);
                pizzaPanel.Controls.Add(textBox);
                pizzaPanel.Controls.Add(regex);

                pnlProducts.Controls.Add(pizzaPanel);
            }
        }
        ...
        private ArrayList GetOrders()
        {
            ...
            foreach (TextBox textBox in textBoxList)
            {
                ...
                if (textBox.Text != "")
                {
                    ...
                    if (amountOfOrders > 0)
                    {
                        Pizza pizza = ConnectionClass.GetPizzaById(Convert.ToInt32(textBox.ID));
                        Order order = new Order(
                            Session["login"].ToString(), pizza.Name, amountOfOrders, pizza.Price, DateTime.Now, false);
