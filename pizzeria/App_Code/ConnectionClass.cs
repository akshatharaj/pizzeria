...
public static class ConnectionClass
{
    ...
    {
        string connectionString = ConfigurationManager.ConnectionStrings["pizzaConnection"].ToString();
        conn = new SqlConnection(connectionString);
        command = new SqlCommand("", conn);
    }

    #region Pizza
    public static ArrayList GetPizzaByType(string pizzaType)
    {
        ArrayList list = new ArrayList();
        string query = string.Format("SELECT * FROM pizza WHERE Type LIKE '{0}'", pizzaType);
        ...
        try
        {
            ...
            while (reader.Read())
            {
                ...
                string crust = reader.GetString(4);
                string sause = reader.GetString(5);
                string image = reader.GetString(6);
                string review = reader.GetString(7);

                Pizza pizza = new Pizza(id, name, type, price, crust, sause, image, review);
                list.Add(pizza);
            }
        }
        finally
    ...
    {
        string query = String.Format("SELECT * FROM pizza WHERE id =  '{0}'", id);
        Pizza pizza = null;
        ...
        try
        {
            ...
            while (reader.Read())
            {
                ...
                pizza = new Pizza(name, type, price, crust, sause, image, review);
            }
        }
        finally
        {
    ...
    {
        string query = string.Format("INSERT INTO pizza VALUES ('{0}', '{1}', @price, '{2}', '{3}','{4}','{5}')",
            pizza.Name, pizza.Type, pizza.Crust, pizza.Sause, pizza.Image, pizza.Review);
        command.CommandText = query;
        command.Parameters.Add(new SqlParameter("@price", pizza.Price));
        try
        {
            conn.Open();
    ...
    public static string RegisterUser(User user)
    {
        ...
        try
        {
            ...
            if (amountOfUsers < 1)
            {
                ...
                query = string.Format("INSERT INTO users (Name, Password, Email, User_type) VALUES ('{0}', '{1}', '{2}', 'guest')", user.Name, user.Password,
                                      user.Email);
                command.CommandText = query;
                command.ExecuteNonQuery();
                return "Registration Succesful! Please Login";
    ...
    {
        string query = string.Format(@"UPDATE [PizzaDB].[dbo].[orders]
                                       SET orderShipped = 1
                                       WHERE client = @client
                                       AND date = @date");
        try
